  ...
  return(
    ...
    {showConfirm && <div onClick={()=>{setShowConfirm(false)}} className="top-0 left-0 z-50 fixed bg-black/50 w-screen h-screen flex items-center justify-center">
      <Card className="px-10 py-4 flex flex-col gap-6">
        ...
        {invitee.quantity > 1 && <Button onClick={(e: MouseEvent<HTMLButtonElement>)=>{answer(e,true, 1)}} className="bg-[#2C3639] hover:bg-[#2C3639]/90">Sí, Voy sol{invitee.gender === 'male' ? 'o' : invitee.gender === 'female' ? 'a' : '@'}</Button>}
